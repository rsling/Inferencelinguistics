...
While we can't think of a scientific experiment where the mode of a binary variable is a sufficient statistic, it is in presidential run-off votes in France.
...
A histogram for a variable with discrete levels is usually displayed with spaces between the bars (compare Figure~\ref{fig:hist} against this), and it's often called a \Key{bar plot} instead of a histogram.
...
In the example, $m=4$.
The \Key{mode} is just the most frequent value, much like in the binary case, and we would like to point out rather informally what the necessary steps to find it are.
Let's name the results of the case counting example (\ie, the sample) \xsample\ and regard it as a \Key{tuple}, customarily specified in angled brackets $\langle~\rangle$:
...
In that respect, we could use a \Key{set} (which is unordered by definition) instead of a tuple, symbolised by curly brackets $\{~\}$ instead of angled brackets.
...
We need to convert \xsample\ into a structure that provides the counts for each level.
The resulting structure should be something like Table~\ref{tab:nominal}, a mapping from the levels of the variable to their counts.
Then, we need a function that goes through the table, finds the \Key{maximum} value among the counts and retrieves the corresponding label.
While this might sound trivial, these are the steps that your statistics software must go through in order to find what you're looking for.
