...
Then, we find the label corresponding to the highest number from that table, denoted $max(\xcounts)$.
That's it.
...
<<ordinalhist, fig.cap=paste0("Histograms of counts of an ordinal five-level variable (acceptability ratings) with the mode highlighted in colour"), echo=F, fig.pos="t", out.width="85%", cache=T>>=
  h <- barplot(ex.rating, xaxt = "n", xlab = "", ylab = "Count", main ="", border=F, ylim = c(0, 6),
               col = c(rep("gray", 2), the.lightcols[1], rep("gray", 2)))
...
The gray line is just a helper line as measurements are, of course, discrete.
The green dashed lines marks the middle point of the ordered sample in the sense that half the ordered sample lies to its left and the other half to its right.

<<ordinalsort, fig.cap=paste0("Raw data points of a sorted sample of an ordinal value; the green line markes the position of the median"), echo=F, fig.pos="t", out.width="85%", cache=T>>=
  ...
  lines(c(5.5, 5.5), c(1, 4.9), lty = 2, lwd = the.lwd, col = the.lightcols[1])
@

The value that lies in the middle of the ordered sample (marked by the coloured dashed line in Figure~\ref{fig:ordinalsort}) is the median.
...
For ordinal scales, we strongly advise not to use the latter solution (conversion to numbers) as it creates a false impression (see Section~\ref{sec:numeric}).
