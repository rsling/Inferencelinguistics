...
<<binaryhist, fig.cap=paste0("Histogram (or barplot) of counts of a binary variable with the mode highlighted in colour"), echo=F, fig.pos="t", out.width="85%", cache=T>>=
  ex.number <- c(4,5)
  h <- barplot(ex.number, xaxt = "n", xlab = "", ylab = "Count", main ="", border=F, col = c("lightgray", the.lightcols[1]), ylim=c(0, 6))
...
A histogram for a variable with discrete values is often displayed with spaces between the bars, and it's often called a \Key{barplot} instead of a histogram.
...
Apparently, the variable \textit{Case} measuring grammatical case in German has four values which have no empirically significant order.
...
A variable which has more than two discrete values with no measurable ordering is called a \Key{nominal variable}.
...
It would be perfectly okay to disregard the order of the events because we (albeit incorrectly, see page~\pageref{abs:independent}) assume that the events are independent.
In that respect, we could use a \alert{set} (which is always unordered) instead of a tuple, symbolised by curly brackets $\{\~\}$ instead of angled brackets.
...
Tuples are sometimes called \textit{lists} in Python, \textit{vectors} in R, \textit{arrays} in more substantive programming languages like C++, etc.

Let \xcounts\ denote the counts of the occurrences of the possible values in the sample.
It's a mathematical representation of what's shown in Table~\ref{tab:nominal}.
It's the label of each possible values, paired with the number of occurrences of that value.
Assuming a function called $max(\cdot)$ that finds the \Key{maximum} value within such a structure and denoting the mode of any sample \xsample\ by \xmode, then:
...
It means that the mode $\widetilde{\ }$ of the sample \xsample\ is obtained by counting the numbers of occurrences of the values of the measured variable, resulting in the table structure \xcounts.
Then, we find the label corresponding to the highest value from that table, denoted $max(\xcounts)$.
That's it.
For the example, it's $\xmode=max(\xcounts)=\mathit{nominative}$ or, in words directly corresponding to the notation:
The mode of the sample is the value occurring most often, namely \textit{nominative}.
...
However, two histograms (or barplots) for the example are shown in Figure~\ref{fig:nominalhist}.
...
<<nominalhist, fig.cap=paste0("Two equivalent histograms of counts of a nominal four-valued variable (grammatical case in German) with the mode highlighted in colour"), echo=F, fig.pos="t", out.width="100%", cache=T>>=
  par(mfrow=c(1,2))

  ex.case <- c(13,1,7,0)
  h <- barplot(ex.case, xaxt = "n", xlab = "", ylab = "Count", main ="", border=F, col = c(the.lightcols[1], "lightgray", "lightgray", "lightgray"), ylim = c(0, 14))
