...
\section{Central Tendency and Typical Values}
...
The gray line is just a helper line for better visual orientation.
...
\label{sub:populationssamples}

% Introduce DGP.
% Proportions being more adequate or "natural" for (infinite) populations than samples.
...
\subsubsection{Why Homogeneity Matters}
...
As we'll discover throughout the book, it's equally important to quantify how homogeneous the values from a sample or a population are.
Let's illustrate the specific kind of homogeneity with two samples.
...
<<varxandy, echo=F, cache=T, out.width="85%", results="asis">>=
  numsumxy <- matrix(
...
<<xyhist, fig.cap=paste0("Histograms of two samples of a numeric variable with n=100"), echo=F, fig.pos="t", out.width="100%", cache=T>>=
  par(mfrow = c(1,2), mar = c(2, 3, 2, 2))
  h1 <- hist(potsx, breaks = 10, xlab = "Milliseconds", ylab = "Count", main ="", border=F, ylim = c(0, 27), xlim = c(50,400))
  h2 <- hist(potsy, breaks = 15, xlab = "Milliseconds", ylab = "Count", main ="", border=F, ylim = c(0, 27), xlim = c(50,400))
  par(mfrow = c(1,1), mar = c(5.1, 4.1, 4.1, 2.1))
@ %

<<xydens, fig.cap=paste0("Density plots of two samples of a numeric variable with n=100"), echo=F, fig.pos="t", out.width="100%", cache=T>>=
  par(mfrow = c(1,2))

  densplo1 <- density(potsx)
  ...
  plot(densplo1, xlab = "Milliseconds", main ="",  bty ="n", ylim = c(0, 0.012), xlim = c(0, 500), lwd = 1)
  lines(densplo1, lwd = the.lwd, col= "darkgray")

  densplo2 <- density(potsy)
  ...
  plot(densplo2, xlab = "Milliseconds", main ="",  bty ="n", ylim = c(0, 0.012), xlim = c(0, 500), lwd = 1)
  lines(densplo2, lwd = the.lwd, col= "darkgray")
...
<<meandist, fig.cap=paste0("Raw data plot of a sample with n=100; distances to mean plotted as vertical lines"), echo=F, fig.pos="t", out.width="85%", cache=T>>=
  p <- plot(pots, xaxt = "n", xlab = "Sorted Data Points", ylab = "Milliseconds", main ="", ylim = c(150, 400), frame.plot = F)
  ...
  .devnull <- lapply(1:10, function(i) {
    ...
    lines(c(i, i), c(pots[i], .mean), lwd = the.lwd,
          col = ifelse(.diff < 0, the.lightcols[3], the.lightcols[1]))
  })
  lines(c(1, 10), rep(round(mean(pots, 0)), 2), lty = 1, lwd = the.lwd+1, col = the.lightcols[2])
  points(1:10, pots, pch = the.pchs[1], cex = 2)
@
...
<<xdistances, echo=F, cache=T, out.width="85%", results="asis">>=
  distxy <- matrix(
    c(pots, paste0(pots, "-", round(.mean, 0)), round(pots-.mean, 0)),
    nrow = 10)
  rownames(distxy) <- paste0("Data point ", 1:10)
  colnames(distxy) <- c("Measurement", "Calculation", "Distance")
...
Simply adding those numbers wouldn't be very helpful.
Do you see why?
The problem is that negative and positive deviations cancel each other out.

\begin{center}
  \begin{math}
    \sum\limits_{i=1}^{n}(x_i-\cfrac{\sum\limits_{i=1}^{n}x_i}{n})=0
  \end{math}
\end{center}

It would be best to make all values positive.
One option would be to take the \Key{absolute} value, which is achieved by discarding the sign.
While this is an option, another one is more appropriate for many further calculations in statistics.
We san also square the distances.
...
It's called the \Key{sum of squares}, and we abbreviate it as $SQ$.%
...
\begin{equation}
  \sum\limits_{i=1}^{n}(x_i-\bar{\xsample})^2
  \label{eq:sq}
\end{equation}

While the $SQ$ is the \textit{total} (summed) squared deviation from the mean, we're rather looking for the average deviation per data point.
...
The larger $n$, the smaller the effect of this correction.
Hence, the sample variance is customarily computed as:

\begin{equation}
  s^2(\xsample)=\cfrac{\sum\limits_{i=1}^{n}(x_i-\bar{\xsample})^2}{n-1}=\cfrac{SQ}{n-1}
  \label{eq:variance}
\end{equation}
