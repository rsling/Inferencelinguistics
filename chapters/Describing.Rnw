...
<<counts, echo=F, cache=F, out.width="85%", results="asis">>=
...
Usually, the more relevant information you provide in your publications, the bettr it is for the reader.}
...
<<binaryhist, fig.cap=paste0("Histogram (or bar plot) of counts of a binary variable with the mode highlighted in colour"), echo=F, fig.pos="t", out.width="85%", cache=F>>=
  ex.number <- c(4,5)
  h <- barplot(ex.number, xaxt = "n", xlab = "", ylab = "Count", main ="", border=F, col = c(the.midgray, the.lightcols[2]), ylim=c(0, 6))
...
<<nominal, echo=F, cache=F, out.width="85%", results="asis">>=
...
<<nominalhist, fig.cap=paste0("Two equivalent histograms of counts of a nominal four-level variable (grammatical case in German) with the mode highlighted in colour"), echo=F, fig.pos="t", out.width="100%", cache=F>>=
  par(mfrow=c(1,2))

  ex.case <- c(13,1,7,0)
  h <- barplot(ex.case, xaxt = "n", xlab = "", ylab = "Count", main ="", border=F, col = c(the.lightcols[2], the.midgray, the.midgray, the.midgray), ylim = c(0, 14))
...
<<ordinalsetup, cache=F, echo=FALSE, include=FALSE, results='asis'>>=
  set.seed(75)
...
<<ordinal, echo=F, cache=F, out.width="85%", results="asis">>=
...
<<ordinalhist, fig.cap=paste0("Histograms of counts of an ordinal five-level variable (acceptability ratings) with the mode highlighted in colour"), echo=F, fig.pos="t", out.width="85%", cache=F>>=
...
<<ratingsmedian, cache=F, echo=FALSE, include=FALSE, results='asis'>>=
...
<<ordinalsort, fig.cap=paste0("Raw data plot of a sorted sample of an ordinal value; the coloured line marks the position of the median"), echo=F, fig.pos="t", out.width="85%", cache=F>>=
...
<<numericsetup, cache=F, echo=FALSE, include=FALSE, results='asis'>>=
  set.seed(2893)
...
<<numericraw, fig.cap=paste0("Raw data plot of a sorted sample of a numeric variable; the coloured line marks the position of the median"), echo=F, fig.pos="t", out.width="85%", cache=F>>=
...
<<numericrawq, fig.cap=paste0("Raw data plot of a sorted sample of a numeric variable; the coloured lines mark the position of the median, the lower and upper quartile, as well as the minimum and the maximum"), echo=F, fig.pos="t", out.width="85%", cache=F>>=
  p <- plot(pots, xaxt = "n", xlab = "Sorted Data Points", ylab = "Milliseconds", main ="", ylim = c(150, 400), frame.plot = F)
  axis(1, 1:10, 1:10, tick = T)
  lines(c(5.5, 5.5), c(150, median(pots)), lty = 1, lwd = the.lwd, col = the.cols[2])
...
<<numericsummary, echo=F, cache=F, out.width="85%", results="asis">>=
  numsum <- matrix(
...
<<numericsummaryx, echo=F, cache=F, out.width="85%", results="asis">>=
...
<<numhist, fig.cap=paste0("Histogram of a sample of a numeric variable with n=100 and a corresponding density plot based on an estimated probability density function"), echo=F, fig.pos="t", out.width="100%", cache=F>>=
  par(mfrow = c(1,2))

  h <- hist(potsx, breaks = 10, xlab = "Milliseconds", ylab = "Count", main ="", border=F, ylim = c(0, 25), col = the.midgray)
...
<<boxplot, fig.cap=paste0("A box plot and a violin plot for a numeric sample with n=100"), echo=F, fig.pos="t", out.width="85%", cache=F>>=
...
<<variancesetup, cache=F, echo=FALSE, include=FALSE, results='asis'>>=
  set.seed(47)
...
<<varxandy, echo=F, cache=F, out.width="85%", results="asis">>=
  numsumxy <- matrix(
...
<<xyhist, fig.cap=paste0("Histograms and density plots of two different samples of a numeric variable with n=100"), echo=F, fig.pos="t", out.width="100%", cache=F>>=
  par(mfrow = c(2,2), mar = c(2, 3, 2, 2))
  h1 <- hist(potsx, breaks = 10, xlab = "Milliseconds", ylab = "Count", main ="", border=F, ylim = c(0, 27), xlim = c(0,500), col = the.midgray)
  h2 <- hist(potsy, breaks = 15, xlab = "Milliseconds", ylab = "Count", main ="", border=F, ylim = c(0, 27), xlim = c(0,500), col = the.midgray)
  ...
  plot(densplo1, xlab = "Milliseconds", main ="x",  bty ="n", ylim = c(0, 0.012), xlim = c(0, 500), lwd = 1)
  ...
  plot(densplo2, xlab = "Milliseconds", main ="y",  bty ="n", ylim = c(0, 0.012), xlim = c(0, 500), lwd = 1)
...
Handling data that are distributed thusly requires much more care compared to data that looks like \xsample, and we'll return to this later in the book.
...
<<meandist, fig.cap=paste0("Raw data plot of a sample with n=100; distances to mean plotted as vertical lines"), echo=F, fig.pos="t", out.width="85%", cache=F>>=
  p <- plot(pots, xaxt = "n", xlab = "Sorted Data Points", ylab = "Milliseconds", main ="", ylim = c(150, 400), frame.plot = F)
...
<<xdistances, echo=F, cache=F, out.width="85%", results="asis">>=
  distxy <- matrix(
...
If we then sum up the squared distances, we get a squared value of the total deviation of the data points from the mean.
It's called the \Key{sum of squares}, and we abbreviate it as $SQ(\xsample)$ in function notation or simply $SQ$.%
...
\begin{equation}
  SQ(\xsample)=\sum\limits_{i=1}^{n}(x_i-\bar{\xsample})^2
  \label{eq:sq}
\end{equation}
...
We can get there by dividing $SQ$ by $n$, which gives us the so-called sample \Key{variance} of \xsample\ denoted by $s^2(\xsample)$ in function notation or simply $s^2$ if it's clear that we're talking about one sample only.
...
The result is called the \Key{standard deviation}, denoted $s(\xsample)$ in function notation or simply $s$:
...
\begin{center}
  \begin{math}
    s^2(\xsample)=\cfrac{\Sexpr{format(sum((round(pots-.mean, 0))^2), scientific = F)}}{\Sexpr{length(pots)-1}}=\Sexpr{round(var(pots), 1)}
  \end{math}
\end{center}
...
\begin{center}
  \begin{math}
    s(\xsample)=\sqrt{\Sexpr{round(var(pots), 1)}}=\Sexpr{round(sqrt(round(var(pots), 1)),1)}
  \end{math}
\end{center}
...
<<varxandyvar, echo=F, cache=F, out.width="85%", results="asis">>=
  numsumxy <- matrix(
