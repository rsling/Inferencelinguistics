...
The maths for calculating probabilities of such sequences of events is exactly the same as for sequences of experiments as long as the sub-events are \Key{independent events}.
...
Once it has been used in a sequence of sentences, however, the probability that it's used again might rise to the levels of words like \textit{technicality} or \textit{opposition}.
...
This is not a necessary condition, and we'll encounter situations where it doesn't hold later in this book.
