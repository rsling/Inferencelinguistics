...
\noindent Hence, $Pr(\Dice{4}\cap\Dice{3})=Pr(\Dice{4})\cdot Pr(\Dice{3})=(1\div 6)\cdot(1\div 6)=1\div 36=0.028$.%
\footnote{It's by pure accident that we ended up with two different results on this page which are numerically identical at 0.028.}
By the way, in all cases discussed so far, the probabilities were evenly distributed among the possible outcomes, which is called a \Key{uniformly random distribution}.
This is not a necessary condition, and we'll encounter situations where it's not the case later in this book.
...
If we applied $\text{sig}\defeq 0.05$ to test whether there is an effect, we'd almost be guaranteed to \textit{reach significance} (which is what people call it when they reach their pre-set sig-level) at 55\% accuracy, albeit the tea-tasting abilities of the participant are middling (to say the least).
