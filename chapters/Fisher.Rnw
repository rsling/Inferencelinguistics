...
The \alert{p-value calculated in Fisher's Exact Test} is the frequentist probability of obtaining a certain result or a more extreme result in simple experiments involving two two-valued variables and counts of their instances.
The \alert{effect strength} for Fisher's Exact Test is a measure of how strongly two variables interact.
Finally, \alert{probabilty distribution functions} are general functions to calculate (and plot) such probabilities that have specific mathematical properties.

Readers should be warned that this chapter is a tad narrative and slow in order to create insight into the basic idea of frequentist inference.
We consider this vital in order to remedy problems with frequentist practice (not with the frequentist philosophy).
Take your time and be assured that we will up the pace in later chapters.
Also, we promise that we will never talk about tea again in later chapters.
...
Then, we'll return to the questions from the problem statement.
...
If there are a thousand lots in the urn, one of them is a win, all the others are duds, and you draw exactly one, most people know you have a chance of one in a thousand (or 1:1\,000) to win.
...
\begin{center}\begin{math}
  \cfrac{8\cdot 7\cdot 6\cdot 5}{4\cdot 3\cdot 2\cdot 1}=\cfrac{1,680}{24}=70
\end{math}\end{center}
...
In general, we'd assume that there isn't nothing going on.
...
This is all completely wrong, and you should actually find such interpretations surprisingly unwarranted if you followed our argument.
...
Probability only plays a role when there is a confrontation with chance, and before the dice are rolled.
The probability we calculated above \alert{was} the probability of obtaining the result which we then actually obtained \alert{before we conducted the experiment} and \alert{under the assumption that there was no ability, effect, etc. but just pure randomness}.
...
We consider this notion (sometimes also called a \textit{subjective probability} or similar) to be utterly undefined and above all irrelevant to science as science is about how things really are and not about our subjective certainty of how they are.
...
However, everybody (including---I think---Bayesians) agrees that the \alert{frequentist probabilities calculated in this book have nothing to do with ``being certain'' about anything to a quantifiable degree}.
...
At some point I just started calling it an experiment, probably without most people noticing.
...
Interestingly, Carl and his friends repeated the ``experiment'' over and over again, playing the national lottery every week for roughly ten years in total (over five hundred draws) without ever winning any considerable amount of money ever again.
This really makes it look like they were just guessing numbers and got lucky exactly once.
Obviously, repeating experiments (so-called \Key{replication}) is a very good way of testing inferences made based on unexpected outcomes further.
...
On the contrary, the p-value quantifies the expectedness of the actual result under a given Null (\ie, by chance) with perfect precision.
...
\begin{center}\begin{math}
  \cfrac{\dbinom{890}{90}\dbinom{210}{10}}{\dbinom{1,100}{100}}+\cfrac{\dbinom{890}{91}\dbinom{210}{9}}{\dbinom{1,100}{100}}+\dots+\cfrac{\dbinom{890}{100}\dbinom{210}{0}}{\dbinom{1,100}{100}}\approx 0.008
\end{math}\end{center}
...
If \textit{sleep} did indeed behave like the totality of all other verbs with respect to passivisation in the fictional corpus described above, 100 instances of \textit{sleep}, randomly sampled from a corpus would still lead to just 10 or less passives in 0.8\% of an infinite or at least very long sequence of trials.
If my grandfather and his friends weren't prescient, they'd still win the big prize in 0.00000715\%  of an infinite or at least very long sequence of draws.

In the case of the corpus study, we had a specific substantive hypothesis generated by many theories of syntax and the lexicon in mind, namely that unaccusative verbs cannot be passivised or are at least very rarely passivised.
...
The test would be a \Key{two-sided test} because it tests for deviations to both sides.
As we'll see in Section~\ref{sec:hypergeometric}, in this case we'd get $p\approx 0.016$.
...
If $q_{\textsc{passive}(\textit{sleep})}$ denotes the proportion of passives of \textit{sleep} and $q_{\textsc{passive}(\neg\textit{sleep})}$ denotes the proportion of passives of all the other verbs, then the mathematical formulation of the Null is:
...
<<arrest, echo=F, cache=F, out.width="85%", results="asis">>=
  ...
  arrest.cont %>%
    as.data.frame.matrix %>%
    kable(align = c("c", "c"), booktabs = T,
          caption = "The fictional result for the voice distribution of \\textit{arrest}",
          label = "arrest") %>%
    add_header_above(c(" ", "Voice"=2), bold = T) %>%
    group_rows(group_label = c("Verb"), start_row = 1, end_row = 2) %>%
    column_spec(4, bold = T) %>%
...
\begin{center}\begin{math}
  \cfrac{\dbinom{869}{69}\dbinom{231}{31}}{\dbinom{1,100}{100}}+\cfrac{\dbinom{869}{68}\dbinom{231}{32}}{\dbinom{1,100}{100}}+\dots+\cfrac{\dbinom{869}{0}\dbinom{231}{100}}{\dbinom{1,100}{100}}\approx 0.009
\end{math}\end{center}
...
However, the directionality involved in formulating the Null is obviously informed by the substantive hypothesis.
...
Assume that Fisher and Ms Bristow, after fiercely discussing the theoretical foundations of her potential tea-tasting abilities after the first experiment (as one should), decide to repeat the experiment but with a much larger sample.
...
In this second experiment, the results are similar to those reported in Table~\ref{tab:fisherone} inasmuch as she guessed 75\% of the tea-first cups correctly, and she wrongly detected 25\% of the milk-first cups incorrectly as tea-first cups.
However, if we calculate the frequentist probability of obtaining this result or an even better result without tea-tasting abilities by chance (the p-value), we get:%
...
\begin{center}
\begin{math}
  \cfrac{\dbinom{40}{30}\dbinom{40}{10}}{\dbinom{80}{40}}+\cfrac{\dbinom{40}{31}\dbinom{40}{9}}{\dbinom{80}{40}}+\dots+\cfrac{\dbinom{40}{40}\dbinom{40}{0}}{\dbinom{80}{40}}\approx 7.44\cdot 10^{-6}
\end{math}
\end{center}

\noindent Do you recognise the exponential notation?
With a negative exponent of $-6$, it means that you have to move the point 6 digits to the left, hence $7.44\cdot 10^{-6}=0.00000744$.
Although the rate of accuracy is the same, the p-value is much lower than the p-value of 0.24 corresponding to Table~\ref{tab:fisherone}.
...
An urn once again serves as a good example of this effect.
Let's consider a situation where  someone claims that an urn at a funfair is unfair inasmuch as it does not contain the advertised number of winning lots.
The showman claims there are 1,000 lots in the urn, 300 of them winners, but you suspect that there are much less winners.
To test his claim that the urn contains 30\% winning lots, you agree to draw a sample of 10 lots.
If you end up with 1 winning lot and 9 duds, it wouldn't be (again, intuitively) much evidence of anything because such a small sample is generally not trustworthy.
If you drew a sample of 100 lots, however, and you ended up with 10 winners and 90 duds, the showman would be in trouble.%
\footnote{An appropriate way to analyse this exact situation (the Binomial Test) will be demonstrated in Chapter~\ref{sec:nonparametric}.
The overall topic of sample size and sample accuracy will be discussed in Chapter~\ref{sec:confidence}.}
In the same vein, the Tea Tasting Marathon can be compared to the small-sample Tea Tasting Experiment.
...
We simply \alert{increase our chances of detecting an effect} (indirectly, by rejecting the Null).
...
Similarly, even if \textit{sleep} is an unaccusative verb, we expect it to pop up in the passive voice from time to time, either because we consider grammar to be probabilistic (many usage-based approaches), or because we attribute such occurrences to performance effects (typically generative and formal approaches with a psycholinguistic interest in processing).
...
The fictional corpus study about passives was introduced from the beginning with caveats regarding marginal contexts where even unaccusative verbs could be passivised, etc.
...
Nobody expects to never ever find any unaccusative verb in the passive voice in the output of real speakers, and nobody expects to get unanimous and perfect rejection of all passives of unaccusative verbs in a rating experiment.
Therefore, the effect (=~unaccusativity leads to reduced passivisation) has a certain \Key{effect strength}.
The effect strength is a property of the real-world phenomenon we're trying to capture with our experiment.
It is thus crucially different from the sample size, which is merely a property of our experiment.
...
The effect strength, on the other hand, changes the distribution of potential p-values.
...
This often gives the impression that it's legitimate to increase the sample size until we finally catch that pesky effect.
...
Only a true understanding of frequentist inference helps in applying it correctly in our everyday research.
...
We agree that it might sound confusing to speak of the probability of obtaining a p-value, because the p-value feels like it should be a probability itself.
...
A p-values clearly isn't a probability itself.
...
There are introductions based on and devoted to simulations (see \citealt{VasishthBroe2011} or even better and more advanced \citealt{CarseyHarden2014}), but we'll keep it short since we're not trying to teach you how to do simulations yourself.
Modern computers are powerful machines, and they have very good random number generators.%
\footnote{Here, ``good'' means that they approach true randomness very well without reconstructible patterns in the sequence of generated numbers.}
Therefore, we can use them to simulate processes that generate data, where a process can be anything from Ms Bristow labelling cups of tea to verbs occurring in the active and passive voice.
In the simulation, we have the luxury of controlling all numerical properties of a simulated real world.
...
Then, the simulation spits out any amount of data generated according to our specifications, and by analysing those data with statistical tests, it is possible to see how the tests perform in uncovering the true nature of the process generating the data, \ie, the true nature of reality.
While simulations cannot be used to find out much about the actual real world, there are two advantages to this approach.
First, the data are virtually free.
Anyone who has ever conducted an experiment or a corpus study know that empirical work is tedious and expensive.
...
Therefore, it is utterly useless to test or demonstrate properties of statistical methods.
On the contrary, we rely on our statistical methods and our knowledge of them when doing empirical work in order to make valid inferences.
Indeed, simulations are used to test the properties of our statistical tools, and they are used to demonstrate these properties in teaching statistics.
...
Furthermore, we simulate a classic Tea Tasting Experiment with four tea-first and four milk-first cups.
...
Since the appropriate test is single-sided, the p-value is different from the one corresponding to Table~\ref{tab:fisherone}, which looks similar superficially.
It's $\Sexpr{round(fisher.test(table(tea0, tea.exp), alternative = "gr")$p.value, 3)}$.
Do you see why this is a desirable result?

However, the purpose of running simulations is usually not to run them once but to run them very often.
...
% h <- hist(sim.guess.p, breaks = 10, xaxt = "n", xlab = "P-Values")
% text(h[["mids"]], rep(20, 10), labels = h[["counts"]])
% axis(1, seq(0, 1, 0.1), seq(0, 1, 0.1))

\section{Probability Distributions}\label{sec:hypergeometric}

\section{Some Afterthoughts on Tests and Collostructions}
