...
\noindent In the long run (repeating the experiment over and over again), even a random guesser would classify 3 cups correctly with a probability 0, 0.23, which corresponds to 23\% of all experiments.
...
The maths has been established, and this section is about the interpretation of the resulting number.
...
Interestingly, Carl and his friends repeated the pseudo-experiment over and over again, playing the national lottery every week for roughly ten years in total (over five hundred draws) without ever winning any considerable amount of money again.
...
Alas, the world isn't like that.
...
The whole purpose of statistics is to find out and quantify the probabilities for certain values of those variables.
...
If we plot the probabilities (y-axis) of all possible outcomes of $k$ successes (x-axis) while keeping $K$, $N$, and $n$ fixed, we get the graph in Figure~\ref{fig:hypergeom844} (left panel).
