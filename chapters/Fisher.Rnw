...
The frequentist probability of rolling six pips with a fair die is $1\div 6$ (or roughly 0.17) before you roll the die.
...
It becomes a fact.
We apply this reasoning to lotteries, parlour games with self-proclaimed psychics, and corpus studies.
...
In other words, those outcomes would be rare if we repeated the experiment over and over again, and if only pure randomness were in control of the outcomes (rather than some influencing factor of interest).
To quantify these notions and make some careful inferences based on the quantification, we introduce several mathematical concepts.
...
\alert{Probabilty distribution functions} are general functions to calculate (and plot) such probabilities for specific parameters.
...
However, a typical type of misunderstandings and ill ways of phrasing the mathematical results of frequentist statistics must be mentioned immediately.
...
Everybody agrees, however, that the \alert{frequentist probabilities calculated in this book have nothing to do with being certain about anything to a quantifiable degree}.
...
In that show, contestants regularly had to choose one door out of three, and there was a big prize behind one of the doors and duds called \textit{Zonk} behind the other two.
...
How many tea-first cups were chosen correctly relative to the number of tea-first cups in the set-up of the experiment, and the same for milk-first cups?
...
If the result obtained in the experiment has a very low frequentist probability under the assumption that the Null is true (\ie, it's an unexpected result), the experiment is assumed to lend some limited support for the theory (or rather a minor hypothesis which is part of the theory).
