...
For example, a 95\% error interval ($a=0.95$) means that before the sample is drawn, it has a probability of 0.95 to actually include the true mean.
Once it is drawn, it either includes the true mean, or a (relatively) rare event has occurred.
In the long run, it is guaranteed that a proportion of $a$ intervals thus calculated ($a*100$\%) include the true mean.
...
The term \textit{error interval} reflects the fact that all we do is control the error rate of the estimation.
