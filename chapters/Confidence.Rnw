...
<<rts, fig.cap=paste0("Raw data plot of a single random sample with n=100 from a DGP that has μ=", rt.mean, " and σ=", rt.sd), echo=F, out.width="85%", fig.pos="t", cache=T>>=
...
<<normal, fig.cap=paste0("Theoretical population distribution for a normal distribution with μ=0 and σ=1"), echo=F, out.width="85%", fig.pos="t", cache=T>>=
...
<<rtsnormal, fig.cap=paste0("Density of a normal distribution with μ=", rt.mean, " and σ=", rt.sd), echo=F, out.width="85%", fig.pos="t", cache=T>>=
...
<<meandensten, fig.cap=paste0("Density of the distribution of the means at n=20 with μ=", rt.mean, " and σ=", rt.sd, "; are at μ±σ is highlighted"), echo=F, out.width="85%", fig.pos="t", cache=T>>=
...
<<cisim, fig.cap=paste0("Empirical error intervals from ", reps.ci ," simulated samples; ones that don't contain the true mean marked in red; μ=", rt.mean, ", σ=", rt.sd, ", z=", z.ci, ", n=", n.ci), echo=F, out.width="85%", fig.pos="t", cache=T>>=
  par(mar=c(1,2,1,1))
  ylim.ci <- c(90, 150)
...
<<fourcisims, fig.cap=paste0("Simulations of empirical error intervals for different z and n with 100 replications each"), echo=F, out.width="85%", fig.pos="t", cache=T>>=
...
<<histcompare, fig.cap=paste0("Histograms of 1000 proportions measured in samples (left) and 1000 sample means (right) at the same sample sizes"), echo=F, out.width="100%", fig.pos="t", cache=T>>=
...
<<plotnormalone, fig.cap=paste0("Dissecting the density function of the Normal Distribution"), echo=F, out.width="85%", fig.pos="t", cache=T>>=
  mmu <- 120
  ssigma <- 20
...
<<plotnormaltwo, fig.cap=paste0("Dissecting the density function of the Normal Distribution; final version"), echo=F, out.width="85%", fig.pos="t", cache=T>>=
  par(mfrow=c(1,1))
