...
\subsection{Error Intervals for Normal Numeric Measurements}
...
\subsubsection{Theoretical Error Intervals}
...
\subsubsection{Empirical Error Intervals}
...
Then, as the inverse of the theoretical error interval, the empirical error interval will contain the true value in $a$ of all cases (as  $z_a$ corresponds to $a$).
Hence, we don't already have to know the truth but still use error intervals.
To make absolutely clear, what an error interval is, we have simulated it.
...
Think about it:
We used the simulations of a known population to generate samples, then we took the samples and calculated the error interval \textit{as if we didn't know the population} to see whether the error interval works as we think it should do.
It's not as good as a mathematical proof, but it relly helps in understanding what statistics is about.
...
<<cisim, fig.cap=paste0("Empirical error intervals from ", reps.ci ," simulated samples; ones that don't contain the true mean marked in red; μ=", rt.mean, ", σ=", rt.sd, ", z=", z.ci, ", n=", n.ci), echo=F, out.width="85%", fig.pos="t", cache=F>>=
  par(mar=c(1,2,1,1))
  ylim.ci <- c(90, 150)
...
A error interval with $a=0.95$ is called the \textit{95\% confidence interval} (and similar for other values of $a$).
...
We call this the \Key{safety of an estimate}:
...
This leads to a higher rate of correct estimates, but the estimates' \Key{precision} decreases.
We call precision the narrowness of the error interval:
The narrower the interval, the higher the precision.

First, let's calculate empirical error intervals with $z=1.96$ and $z=2.58$, before looking at some more illustrative plots.
...
<<fourcisims, fig.cap=paste0("Simulations of empirical error intervals for different z and n with 100 replications each"), echo=F, out.width="85%", fig.pos="t", cache=F>>=
...
For example, a 95\% error interval ($q=0.95$) means that before the sample is drawn, it has a probability of 0.95 to actually include the true mean.
...
The distribution of the proportions from samples with very low sample size show obvious banding and look irregular.
...
Keep in mind:
These are plots of the distribution of proportion and means calculated from a lot of (simulated) samples, not plots of raw individual measurements.

<<histcompare, fig.cap=paste0("Histograms of 1000 proportions measured in samples (left) and 1000 sample means (right) at the same sample sizes"), echo=F, out.width="100%", fig.pos="t", cache=F>>=
  par(mfrow=c(3,2), mar=c(5,5,1.5,1))

  plot(density(unlist(berry.sims["5"]), kernel = "cosine"),
       main = "n=5", xlab = "", ylab = "Density", bty="n", yaxt = "n",
       lwd = the.lwd, col = the.lightcols[[2]], xlim=c(-0.25, 1))
  plot(density(unlist(rt.sims["5"]), kernel = "cosine"),
       main = "n=5", xlab = "", ylab = "", bty="n", yaxt = "n",
       lwd = the.lwd, col = the.lightcols[[1]], xlim=c(90, 150))
...
Most importantly, however, there is virtually not distinction between the distributions proportions at $n=20$, even though the proportions reflect counts of a discrete variable and the means derive from a continuous numerical variable.
...
<<propdist, fig.cap=paste0("Proportions from samples with n=20 with different true population proportions ρ"), echo=F, fig.pos="t", out.width="85%", cache=T>>=
  par(mfrow=c(1,2))
  berry.prob <- 0.1
  n <- 20
  berry.reps <- 1000
  ...
  .sample <- unname(unlist(lapply(1:berry.reps, function(x) {berries(n)})))
  points(.sample,
         pch = 19, col = alpha(the.cols[2], 0.4), cex = 1.5)
  ...
  points(.sample,
         pch = 19, col = alpha(the.cols[3], 0.4), cex = 1.5)

  par.defaults()
@

We hope you noticed what's going on there.
At $\rho=0.5$, the sample proportion deviates from the true propoertion into the negative and the positive direction.
...
The square root conserves this principle property.

<<qnotq, fig.cap=paste0("Proportions q (x-axis) plotted against q⋅(1-q) (y-axis) and its square root"), echo=F, fig.pos="t", out.width="85%", cache=T>>=
  .qnotq <- seq(0, 1, 0.01)
  plot(.qnotq, .qnotq*(1-.qnotq), type = "l", lwd = the.lwd, ylim = c(0, 0.55),
       col = the.lightcols[[2]], bty = "n", ylab = "", xlab = "q (or ρ)")
  lines(.qnotq, sqrt(.qnotq*(1-.qnotq)), type = "l", lwd = the.lwd,
       col = the.lightcols[[1]], bty = "n", ylab = "q⋅(1-q)", xlab = "q")
  legend("topleft", legend = c("q⋅(1-q)", "√[q⋅(1-q)]"), bty = "n",
         col = c(the.lightcols[[2]], the.lightcols[[1]]),
         lwd = the.lwd)
@
...
<<cioverview, fig.cap=paste0("Width of error intervals for proportions at three sample sizes, three safety levels, and for proportions from 0 to 1"), echo=F, fig.pos="t", out.width="85%", cache=T>>=
  ...
  plot(ci.80.10(seq(0, 1, 0.01)), type = "l", col = the.lightcols[1], lwd = 3,
  ...
  axis(1, at = seq(1, 101, 10), labels = seq(0, 1, 0.1), cex = 1.2,
       cex.axis = 1.2, las = 2)
  lines(ci.80.100(seq(0, 1, 0.01)), col = the.lightcols[2], lwd = 3, lty = 1)
  lines(ci.80.1000(seq(0, 1, 0.01)), col = the.lightcols[3], lwd = 3, lty = 1)
  legend("topleft", legend = c("n=10", "n=100", "n=1000"), lwd = 3, col = the.lightcols,
         bty = "n", cex = 1.2)

  ci.95.10 <- function(p) ci.prop(p = p, n = 10)
  ci.95.100 <- function(p) ci.prop(p = p, n = 100)
  ci.95.1000 <- function(p) ci.prop(p = p, n = 1000)
  plot(ci.95.10(seq(0, 1, 0.01)), ty = "l", col = the.lightcols[1], lwd = 3,
  ...
  lines(ci.95.100(seq(0, 1, 0.01)), col = the.lightcols[2], lwd = 3, lty = 1)
  lines(ci.95.1000(seq(0, 1, 0.01)), col = the.lightcols[3], lwd = 3, lty = 1)
  ...
  plot(ci.99.10(seq(0, 1, 0.01)), ty = "l", col = the.lightcols[1], lwd = 3,
       main = "99%", ylim = c(0, 0.9), bty = "n", cex = 1.2, cex.axis = 1.2,
       cex.main = 1.2, cex.lab = 1.2, yaxt = "n", xaxt = "n", ylab = "",
       xlab = "")
  axis(1, at = seq(1, 101, 10), labels = seq(0, 1, 0.1), cex = 1.2,
       cex.axis = 1.2, las = 2)
  lines(ci.99.100(seq(0, 1, 0.01)), col = the.lightcols[2], lwd = 3, lty = 1)
  lines(ci.99.1000(seq(0, 1, 0.01)), col = the.lightcols[3], lwd = 3, lty = 1)
@
...
<<plotnormalone, fig.cap=paste0("Dissecting the density function of the Normal Distribution"), echo=F, out.width="85%", fig.pos="t", cache=F>>=
  mmu <- 120
  ssigma <- 20
...
<<plotnormaltwo, fig.cap=paste0("Dissecting the density function of the Normal Distribution; final version"), echo=F, out.width="85%", fig.pos="t", cache=F>>=
  par(mfrow=c(1,1))
  plot(xs, (1/sqrt(2*pi*ssigma^2)) * 1/exp((xs-mmu)^2 / (2*ssigma^2)), type = "l",
       xlim = c(60, 180), ylim = c(0, 0.02), bty ="n", xlab = "", ylab = "",
       main = "",
       lwd = 0.5, col = the.lightcols[2])
  arrows(125, 0.005, 150, 0.005, 0.075, col = the.lightcols[3], lwd = the.lwd, code = 3)
  arrows(115, 0.005, 90, 0.005, 0.075, col = the.lightcols[3], lwd = the.lwd, code = 3)
  text(137.5, 0.0055, "σ", col = the.lightcols[3], 0.5)
  text(102.5, 0.0055, "σ", col = the.lightcols[3], 0.5)

  lines(c(120, 120), c(0, 0.0199), col = the.lightcols[1], lwd = the.lwd)
  arrows(60, 0, 115, 0, 0.075, col = the.lightcols[1], lwd = the.lwd)
  text(90, 0.0005, "μ", col = the.lightcols[1], 0.5)
