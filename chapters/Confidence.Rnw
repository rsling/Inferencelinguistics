% !Rnw root = ../main.Rnw
<<setupconfidence, cache=F, echo=FALSE, include=FALSE, results='asis'>>=
...
<<berryone, fig.cap=paste0("Proportions measured in 100 replications at n=1 with a true proportion of 0.3"), echo=F, fig.pos="t", out.width="85%", cache=F>>=
...
<<berrythreetoten, fig.cap=paste0("Proportions measured in 100 replications at n=3, n=4, n=5, and n=10 with a true proportion of 0.3"), echo=F, fig.pos="t", out.width="85%", cache=F>>=
...
<<berrybig, fig.cap=paste0("Proportions measured in 100 replications at n=20, n=50, n=100, and n=1000 with a true proportion of 0.3"), echo=F, fig.pos="t", out.width="85%", cache=F>>=
  par(mfrow=c(2,2), mar = c(2,2,2,2))
...
<<rtsmall, fig.cap=paste0("Means measured in 100 replications at n=1, n=3, n=5, and n=10 with a true mean of σ=120 and a true standard deviation of μ=20"), echo=F, fig.pos="t", out.width="85%", cache=F>>=
  par(mfrow=c(2,2), mar = c(2,2,2,2))
  for (i in c(1,3,5,10)) {
...
<<rtsbig, fig.cap=paste0("Means measured in 100 replications at n=20, n=50, n=100, and n=1000 with a true mean of σ=120 and a true standard deviation of μ=20"), echo=F, fig.pos="t", out.width="85%", cache=F>>=
  par(mfrow=c(2,2), mar = c(2,2,2,2))
  for (i in c(20, 50, 100, 1000)) {
    p <- plot(unlist(rt.sims[as.character(i)]),
              pch = 19, col = alpha("white", 0), cex = 1.5, ylim=c(60,160),
...
<<ztable, echo=F, cache=F, out.width="85%", results="asis">>=
...
<<cisims, cache=F, echo=FALSE, include=FALSE, results='asis'>>=
...
<<propdist, fig.cap=paste0("Proportions from samples with n=20 with different true population proportions ρ"), echo=F, fig.pos="t", out.width="85%", cache=F>>=
  par(mfrow=c(1,2))
  berry.prob <- 0.1
  n <- 20
  berry.reps <- 1000
...
<<qnotq, fig.cap=paste0("Proportions q (x-axis) plotted against q⋅(1-q) (y-axis) and its square root"), echo=F, fig.pos="t", out.width="85%", cache=F>>=
  .qnotq <- seq(0, 1, 0.01)
...
<<cioverview, fig.cap=paste0("Width of error intervals for proportions at three sample sizes, three safety levels, and for proportions from 0 to 1"), echo=F, fig.pos="t", out.width="85%", cache=F>>=
...
<<plotnormaltwo, fig.cap=paste0("Dissecting the density function of the Normal Distribution; final version"), echo=F, out.width="85%", fig.pos="t", cache=F>>=
  ...
  arrows(125, 0.005, 150, 0.005, 0.075, col = the.cols[3], lwd = the.lwd, code = 3)
  arrows(115, 0.005, 90, 0.005, 0.075, col = the.cols[3], lwd = the.lwd, code = 3)
  ...
  arrows(60, 0, 115, 0, 0.075, col = the.cols[2], lwd = the.lwd)
