...
<<normal, fig.cap=paste0("Theoretical population distribution for a normal distribution with μ=0 and σ=1"), echo=F, fig.pos="t", cache=T, fig.height=5>>=
...
<<rtsnormal, fig.cap=paste0("Density of a normal distribution with μ=", rt.mean, " and σ=", rt.sd, " (black); long-run sample means at different sample sizes from that normal distribution"), echo=F, fig.pos="t", cache=T, fig.height=5>>=
...
<<meandensten, fig.cap=paste0("Density of the distribution of the means at n=20 with μ=", rt.mean, " and σ=", rt.sd, "; are at μ±σ is highlighted"), echo=F, fig.pos="t", cache=T, fig.height=5>>=
...
Think about it:
We used the simulations of a known fictitious population to generate samples, then we took the samples and calculated the error interval \textit{pretending we didn't know the population parameters} in order to check whether the error interval works as we think it should.%
\footnote{Yes, in a way we're playing god.
But as opposed to real life, playing god is a very good idea in statistics (if you have a good random number generator).}
...
It's of no big practical consequence, but it's a good thing to keep in mind in order not to misinterpret error intervals.}
...
If safe estimates are of the essence, one should increase $a$ (for example, $a=0.99$ for the 99\% error interval).
...
Hence, if \textit{both a precise and a safe} estimation is required, one should increase $a$ and also increase the sample size $n$.
Once again, error intervals (also called confidence intervals) have nothing to do with specifying the degree of certainty or confidence that the true mean falls within the interval.
}
...
<<histcompare, fig.cap=paste0("Histograms of 1000 sample proportions (left) and 1000 sample means (right) at the same sample sizes"), echo=F, fig.pos="t", cache=T>>=
  ...
  plot(density(unlist(berry.sims["20"]), kernel = "cosine"),
       main = "n=20", xlab = "Proportions", ylab = "", bty="n", yaxt = "n",
       lwd = the.lwd, col = the.darkgray, xlim=c(-0.25, 1))
...
<<qnotq, fig.cap=paste0("Proportions q (x-axis) plotted against q⋅(1-q) (y-axis) and its square root"), echo=F, fig.pos="t", cache=T, fig.height=5>>=
  par.defaults()
  .qnotq <- seq(0, 1, 0.01)
...
<<cioverview, fig.cap=paste0("Width of error intervals for proportions at three sample sizes, three safety levels, and for proportions from 0 to 1"), echo=F, fig.pos="t", cache=T, fig.height=5>>=
