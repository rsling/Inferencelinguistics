...
Do you see why?
We give two columns, one for the single-sided test and one for the two-sided test (see p.~\ref{abs:sided} and immediately Section~\ref{sec:diffciz} below).
...
While we attach a specific statistic---the p-value---to the outcome, it's a matter of debate whether the statistic itself should be interpreted.
You've probably heard of people calling results \textit{significant} when the reach a certain $sig$-level such as $sig=0.05$.
When $p<sig$, \textit{we've reached significance}, but when they obtain $p=0.009$ in the next experiment, suddenly that result is \textit{even more significant}.
...
Nobody can tell you or your community what your threshold needs to be.
...
That's just 5 standard deviations from the expected value under the Null ($z=5$), which corresponds approximately to $p=0.00000029$.
...
That's because there is large uncertainty in the measurements, and the phenomena of interest are beyond microscopic and can only observed very indirectly.
Do you need that level of unexpectedness?
Think about it.
If you're uncertain and your field has just gotten started (such as linguistics, cognitive science, or social psychology), we recommend you don't start above $sig=0.001$.
...
\noindent If not, go back to Chapter~\ref{sec:fisher} and start all over again.

While tests and error intervals are related mathematically (see below), they're entirely different in their interpretation.
You can estimate a value without having any hypotheses at all.
If you just need to know the average rate at which a manufacturing machine produces defective parts, you might take a sample of 1000 parts, count the proportion of defective parts, calculate a safe error interval (at, say, $a=0.99$), and see whether the resulting interval looks acceptable from an economic point of view.
If you do this with all your machines over years and decades, you're estimate should have included the true value in close to 99\% of all times.
Depending on the value of the individual parts, this might be acceptable.
...
<<convzci, fig.cap=paste0("Convergence of z-test and error interval: two-sided test"), echo=F, fig.pos="t", out.width="100%", cache=F>>=
...
<<convzciss, fig.cap=paste0("Convergence of z-test and error interval: single-sided test"), echo=F, fig.pos="t", out.width="100%", cache=F>>=
