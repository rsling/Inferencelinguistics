...
Figure~\ref{fig:simznull} show that each p-value has (minor fluctuations aside) the same probability if the Null is true.
...
Even if the difference between the means is only 5 ($\mu_1=115$ or $\mu=120$), the p-values already collapse close to 0.
...
If the effect is tiny and irrelevant from a theoretical point of view, it's our duty to have a good enough understanding of the statistical methods we use to be able to detect that.
The behaviour of the test itself is, of course, by design.
...
Under such an approach, do you get more, less, or a constant number of significant results when the actual effect strength increases (all other things beaing equal)?
Still under this approach, is your \textit{error rate} constant or does it change with increased effect strength?
The error rate is the percentage of samples which make you erroneously rejected Null when it is true.
