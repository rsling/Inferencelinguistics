...
It's a simulated sample, and the simulation was set up according to the known facts about the overall population.
...
It does that if it's a sufficiently unexpected results under the assumption that the desired inference is not correct, \ie, under the Null.
...
In other words, we would like to \alert{gather evidence in support of a hypothesis H: $\mu_1>\mu$}.
...
First, as a population mean $\mu_1$ is obviously not directly observable as all that we've got are samples.
Rather, it's a hypothesised mean in a population that exists as separate from the known population if the substantive hypothesis is correct.
However, if that population is not substantially different from the known one, then we have $\mu_1=\mu$.
Given that $\mu_1$ is a non-observable, we've got to work with the $\Sexpr{z2.n}$ data points from \xsample\ and the sample mean \xmean\ calculated from them.
...
The standard error of the mean in this case is:
...
If the mean in a population is $\mu$ and the standard deviation is $\sigma$, then the sample means of size $n$ are themselves normally distributed, and the standard error $\ssigma_{\mu}$ is the standard deviation of that normal distribution.
Furthermore, keep in mind that we're talking about the distribution of sample means drawn from the known population.
...
Instead of calculating the individual events as in the Tea-Tasting Experiment, we use the known functions of the Normal Distribution to look up those values.
...
text(x = z.mu+3, y = z2.dnorm.sample(z.mu)/2.5, col = the.cols[2],
     labels = bquote(μ*"="*.(round(mean(z.mu), 2)))
     )
text(x = mean(z2.sample)+2, y = z2.dnorm.sample(mean(z2.sample)),
     labels = bquote(bar("x")*"="*.(round(mean(z2.sample), 2))),
     col = the.cols[1]
     )
...
text(x = mean(z2.sample)+1, y = z2.dnorm.sample(mean(z2.sample))+0.02,
...
The left panel of Figure~\ref{fig:z4} show the PDF of the distribution of sample means with the area under the curve corresponding to results as extreme or more extreme than \xmean\ highlighted.
...
Otherwise, you can calculate the $z$-score and look the p-value up in a table.
...
Hence, $z=\Sexpr{z2.z.rnd}$ is independent of the concrete sample mean and population mean, and it establishes a direct link to the probability we're looking for: another \Key{p-value}.
We can use tables to look up p-values corresponding to z-scores.
An example is shown in Table~\ref{tab:ptable}, and it's the reverse of tables such as Table~\ref{tab:ztable}.
...
% \begin{center}\begin{math}
%   Pr(\xmean|H_0)=p(\Sexpr{z2.z.rnd})=\Sexpr{z2.p.rnd}
% \end{math}\end{center}
%
% Let's go through this step by step.
% First, \alert{$Pr(\bar{x}|H_0)$} reads as \textit{the probability of the sample mean \xmean\ assuming that the Null is true}.
% Remember that the Null was specified as $H_0: \mu_1\leq\mu$, which reads \textit{the mean under the condition of interest} (the mean reaction time in the emergent type of relative clause) \textit{is less than or equal to the population mean} (the reaction time in other relative clauses).
...
%
% \Bigpoint{Interpretation of the P-Value in Z-Tests}{The p-value in a z-test is the \alert{frequentist probability} of drawing a sample $x$ with a mean as extreme as or more extreme than the one that was actually obtained \alert{if the Null were true}. The frequentist probability (p-value) is the probability of an event occurring \alert{before it has actually occurred}. After the sample has been drawn, there is no longer a frequentist probability attached to the event of drawing it.}
