...
Whether an outcome would be unexpected under the Null depends on the phenomenon at hand, the precision of your theory and of your measurement, whether you've ensured that the preconditions for the test were met properly, and probably many other factors.
...
<<tsim, fig.cap="Distributions (density plots) of p-values from simulations of z-Tests and t-Tests with variances estimated from samples; therefore, the z-Tests are inappropriate", fig.height=5, echo=F, fig.pos="t", cache=F>>=
...
You have no precise idea about any population mean to compare a sample mean to, and you've got no idea about the population variance (at least no sufficiently precise idea).
For example, as a socio-phonetician you might be interested in comparing the mean frequency of the F1 formant in articulations of self-described members of US coastal elites to the mean frequency of the same formant in articulations of self-described rural folk from the Midwest in the long [iː] in \textit{weakling}.%
\footnote{As usual in this book, the examples are completely made up and come with a dose of irony.
This stems from our conviction that the examples shouldn't matter for a sound understanding of statistics.
Whether it's mean formant frequencies in hertz or mean daily milk yields of cows in pints doesn't really matter.}
...
f.t <- round((mean(f.elite)-mean(f.rural))/f.se, 2)
...
Hence, you have to work with 20 data points in total.
...
The means are $\xmean=\Sexpr{f.elite.m}$ and $\ymean=\Sexpr{f.rural.m}$.
The standard deviations are $s_{\xsample}=\Sexpr{f.elite.sd}$ and $s_{\ysample}=\Sexpr{f.rural.sd}$.
Indeed, the formant frequency differs by \Sexpr{f.elite.m-f.rural.m} Hz.
...
Under the Null, they are identical ($\mu=\mu_1$).

This leads us nicely to the maths we need.
The t-Test, like the z-Test, is based on subtracting a population mean from sample means.
...
Furthermore, we assumed that the sample means are normally distributed, and hence $\xmean-\mu$ should also be normally distributed.%
\footnote{Notice that the means are really still normally distributed, even in the t-Test.
...
The refreshingly simple logic and maths described above only work as expected if the variances $\sigma$ and $\sigma_1$ from both populations are equal.
...
The variance of the difference score $\xmean-\ymean$ obviously comes from \textit{two} means (estimated from the samples).
...
Hence, the standard error adds these two error components.
...
This unreliability disappears when the total number of data points from \textit{both} samples gets larger.
...
\noindent Let's apply all of this to our example:
...
This doesn't even meet the lowest standard for a surprising result under the Null ($sig=0.05$), and hence we do not conclude anything from this result.
...
We know by now that larger samples represent (on average) a population or a DGP better than smaller samples.
It is never the case that sameller samples lead to better estimates of any population parameter (so far, mostly mean and variance) than larger samples.
...
The resulting strategy is this:
...
Then, we use this variance estimate to introduce two error components in the formula of the standard error, which depend on their respective sample sizes.
The best possible estimate of the variance $s^2_{\xsample,\ysample}$, the so-called \Key{pooled variance} is given by:
...
Notice that we couldn't simply pool the samples here, as each error component depends specifically on the size of one of the two samples.
...
<<pvarsim, fig.cap=paste0(pvar.reps, " simulations of standard errors for mean differences: calculated with simply added variances (gray squares) and calculated with pooled variances (blue dots and orange dots); sample sizes are ", pvar.n.x, " and ", pvar.n.y, "; the true SE is ", pvar.set, " (green line)"), fig.height=5, echo=F, fig.pos="t", cache=F>>=
...
\begin{center}
  \begin{math}
    \ssigma_{\xmean-\ymean}=\sqrt{\cfrac{\Sexpr{pvar.sd^2}}{\Sexpr{pvar.n.x}} + \cfrac{\Sexpr{pvar.sd^2}}{\Sexpr{pvar.n.y}}}=\Sexpr{pvar.set}
  \end{math}
\end{center}

In each replication, we calculated the standard error incorrectly by just adding the individually computed error components (as if the samples were of the same size, Equation~\ref{eq:setwo}) and correctly using the pooled variance (Equation~\ref{eq:setwopooled}).%
...
This illustrates that with pooled variance, the mostly less accurate variance estimate from the smaller sample gets fewer chances to drag down the precision of the on average more precise variance estimate from the larger sample.

Except for a short in-depth discussion of the distribution of mean differences, this concludes our introduction to the z-Test and the t-Test.
...
Table~\ref{tab:tconds} summarises the preconditions that must be met for the tests discussed in this chapter to be applicable.
If they are not met, inferences will be marred as the error rates you hope to control will not be at the level you think they are.
...
\begin{table}
  \caption{\label{tab:tconds}Necessary preconditions for z-Tests and t-Tests}
  \centering
  %\resizebox{\textwidth}{!}{%
  \begin{tabular}{lccc}
    \toprule
...
Some of these requirements have not been mentioned explicitly or very prominently.
Let's go through them, mentioning solutions for when they're not met.

\noindent\ding{192}~For any of the tests of mean differences (such as z-Tests and t-Tests) the mean has to be computable, which is only the case for numerical measurements.
In principle, there's nothing more to say about that.
However, we'd like to tell a story:
We know of a paper in linguistics (which we gracefully refuse to reference) where a theoretical syntactician and an empiricist demonstrated something very convenient for experiments where participants were asked to produce acceptability ratings for sentences.%
\footnote{For non-linguists, let us explain that linguists sometimes ask naive speakers to rate sentences for their acceptability.
...
Alternatively, they are asked to rate it on an absolute scale or on a scale relative to other sentences, which results in more or less numerical measurements.}
...
When, for example, you have 10 sentences for condition A and 10 sentences for condition B, then you can take the mean of the 10 proportions of \textit{acceptable} ratings for condition A and B and perform a t-Test.
Mind-boggling!
Besides the legions of devils that lie in the details with this approach, consider this:
They tried this out in a single instance, and the inferences were the same for the mistreated binary ratings and the numerical ratings.
You know enough about frequentist thinking by now to know that you cannot test whether you can break the rules by trying it out once.
...
Please go back to Figure~\ref{fig:tsim} and see what might happen if you violate the assumptions.
Frequentism is all about the control of error rates, and you need to obey the rules (meet the preconditions of your tests) to ensure that your error rates are what you think they are.
\textit{We tried it out once, and it worked!} is outright ridiculous.

\noindent\ding{193}~The normality of means underlies our entire logic of inferences about means.
...
\noindent\ding{194}~Technically speaking, the data themselves (reaction times, formant frequencies, sentence lengths, etc.) don't need to be normally distributed.
Only the means themselves must fulfil this requirement.
Interestingly, there is a fundamental result (which at first appears very counterintuitive to many) which roughly speaking states that sample means drawn from any underlying distribution will be normal at sufficiently large sample sizes.
...
<<lognormal, fig.cap="Left: idealised discrete distribution of sentence lengths measured in words; Right: a continuous Lognormal Distribution corresponding to the left panel", fig.height=5, echo=F, fig.pos="t", cache=F>>=
...
In linguistics---and especially in corpus linguistics---, many data are not normally distributed.
Because many data are counts or frequencies, their distributions are discrete anyway, but they often skewed or diverge from the Normal Distribution in other ways.
...
Or you can increase the sample size and rely on the CLT to make the sample means normally distributed.
...
\noindent\ding{195}~We'll keep this one short as we'll return to it later in the book anyway.
However, for inferences to be valid, each measurement in your sample(s) must represent an independent random event (see Section~\ref{sec:probability}).
This condition is violated very easily, but once again, it's not necessarily a statistical matter whether it's violated or not.
...
taking measurements in a temporal sequence (which might violate independence through priming or repetition effects if you're measuring reactions of humans);
...
You get groups of ratings per participant, sequences of measurements where the previous one affects the subsequent one, groups defined by lexical items, groups defined by styles of writing.
We'll discuss advanced techniques of taking care of grouping (but not temporal sequences) in Chapter~\ref{sec:mixed}.
However, in simple tests like the t-Test, such grouping can lead to a situation where the distribution of variance in the data is more complex than the test assumes, which (again) means that your actuall error rates might be off from the nominal error rates.
...
\noindent\ding{196}~This is straightforward.
The z-Test and the One-Sample t-Test require that the population mean (the reference, if you will) is known beyond doubt.
...
\noindent\ding{197}~Again, this is a very straightforward matter.
...
\noindent\ding{198}~A similar thing holds for the Two-Sample t-Test with respect to the variances in the two (potential) populations.
They must be equal, and you need to be sure of it.
If in doubt, use Welch's variant of the t-Test.

\noindent\ding{199}~This is a variation on the theme from \ding{195}.
...
If you have data that is structured like this, look up the t-Test for Repeated Measures.
...
<<ndiff, fig.cap=paste0("Left: Density plots of ", ndiff.reps, " random draws each from a Normal Distribution with mean μ=3 and σ=1 (blue), from another Normal Distribution with mean μ₁=1 and σ₁=1 (blue) and the subtraction of random pairs from the second from the first; Right: the theoretical distributions corresponding to the left panel"), fig.height=5, echo=F, fig.pos="t", cache=F>>=
...
Concretely, we subtracted two random variables $\xmean-\ymean$.
...
The left panel provides a density plot of \Sexpr{ndiff.reps} random draws (simulations, if you wish) of two normally distributed random variables:
