...
Table~\ref{tab:tconds} summarises the pre-conditions that must be met for the tests discussed in this chapter to be applicable.
...
\begin{table}
  \caption{\label{tab:tconds}Necessary pre-conditions for z-Tests and t-Tests}
  \centering
  %\resizebox{\textwidth}{!}{%
  \begin{tabular}{lccc}
    \toprule
    ...
    \midrule
    The data are normally distributed.  & \CellBlue\ding{51}       & \CellBlue\ding{51}  &\CellBlue\ding{51}   \\
    The measurements are independent.   & \CellGreen\ding{51}      & \CellGreen\ding{51} & \CellGreen\ding{51} \\
    The population mean is known.       & \CellBlue\ding{51}       & \CellBlue\ding{51}  & \Dim                \\
    The population variance is known.   & \CellGreen\ding{51}      & \multicolumn{2}{c}{\Dim}                  \\
    The population variances are equal. & \multicolumn{2}{c}{\Dim} & \CellBlue\ding{51}                        \\
    The samples are independent.        & \multicolumn{2}{c}{\Dim} & \CellGreen\ding{51}                       \\
    \bottomrule
  \end{tabular}
  %}
\end{table}
...
\subsubsection{\Indepth\ Mean Differences Are Normally Distributed}\label{sec:normaldiffs}
