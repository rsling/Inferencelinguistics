...
<<tsim, fig.cap="Simulations of z-Tests and t-Tests with variances estimated from samples; therefore, the z-Tests are inappropriate", fig.height=5, echo=F, fig.pos="t", cache=F>>=
...
Whether it's mean formant frequencies in hertz or mean daily milk yield of cows in pints doesn't really matter.}
...
In other words, by hypothesis there are two DGPs and thus two populations, one corresponding to coastal speakers (with true mean $\mu$) and one corresponding to speakers from the Midwest (with true mean $\mu_1$).
...
Conveniently, it turns out that results of subtracting \textit{differences of population means} from  \textit{differences of sample means} are also normally distributed or t-distributed if the individual statistics are normally distributed or t-distributed.
Hence, we can calculate a t-statistic for two samples as follows.
The equation implements the logic of the z-Test and the t-Test based on the \textit{distance between a sample mean} (\textit{difference}) \textit{and a known population mean} (\textit{difference}):
...
\noindent We'll return to the question of how to calculate the standard error $\ssigma_{\mu-\mu_1}$ presently.
For the moment, please believe us that there is an appropriate standard error for mean differences.
The standard error aside, Equation~\ref{eq:tinterrim} might not seem to be very useful as we clearly haven't got any idea what the true means $\mu$ and $\mu_1$ are.
...
Alas, there's a catch to be taken into account.
It's related to the calculation of the appropriate standard error, which we referred to above simply as $\ssigma_{\mu-\mu_1}$.
Here's the problem:
...
If that is the case \textit{and the two samples have the same size}, the estimated standard error for the mean differences $\ssigma_{\xmean-\ymean}$ is calculated by just adding the variances of the two samples.
Why do we just add the two?
We are working with two sample estimates of means in order to approximate a difference between two potential populations (albeit populations with identical variance).
Both estimates contribute uncertainty to the calculation, which means that we have two sources of error.
Hence, we use a standard error that adds these two error components (where there is only one error in the One-Sample t-Test):
...
\noindent This estimated standard error $\ssigma_{\xmean-\ymean}$ is used as the estimate of $\ssigma_{\mu-\mu_1}$.
Furthermore, we use a t-Distribution with the appropriate degrees of freedom $\nu_{\xmean-\ymean}$.
Luckily, degrees of freedom can also just be added:
...
\noindent And the t-statistic is calculated as:
...
We won't cover it in this book for reasons of space and focus.
If the two samples are of different sizes but the population variances can be safely assumed to be equal, however, there's a relatively simple solution.
...
If there is a considerable difference in sample size, one sample---the larger one---is thus a better approximation of the common variance of $\mu$ and $\mu_1$.
...
This cannot be the optimal solution, because smaller samples will inevitable drag down larger samples in the long run.
...
We first need to come up with the best estimate of the variance using information from both samples, which---once again---must be identical between the two populations.
Then, we use this variance estimate to introduce two error components in the formula of the standard error.
With $s^2_{\xsample,\ysample}$ standing for the variance estimated from both samples, the standard error is given as:
...
\noindent And the best possible estimate of the variance $s^2_{\xsample,\ysample}$, the so-called \Key{pooled variance} is given by:
...
For comparison, the variance of a single sample $s^2_{\xsample}$ was given in Equation~\ref{eq:variance}, repeated here in condensed form for convenience:
...
We pool both samples to calculate a maximally precise estimate of their variances.
...
Notice that we couldn't simply pool the samples in Equation~\ref{eq:setwopooled} as each error component depends specifically on the size of one of the two samples.
...
<<pvarsim, fig.cap=paste0(pvar.reps, " simulations of standard errors for mean differences: calculated with simply added variances (gray squares) and calculated with pooled variances (blue dots and orange dots); sample sizes are ", pvar.n.x, " and ", pvar.n.y, "; the true SE is ", pvar.set, " (red line)"), fig.height=5, echo=F, fig.pos="t", cache=F>>=
...
legend("topleft", legend = c("SE with pooled variance (closer true SE)",
