...
Conveniently, it turns out that results of subtracting \textit{differences of population means} from  \textit{differences of sample means} are Normal or t-distributed if the individual statistics are also Normal or t-distributed.
Hence, we can calculate a t-statistic for two samples like so:
...
\noindent Keep in mind that the test is always performed under the assumption that the Null is true.
...
Alas, there is a catch to be taken into account, which is related to the calculation of the appropriate standard error, which we referred to above simply as $\ssigma_{\mu-\mu_1}$.
The simple logic and maths described above only work as expected if the variances $\sigma$ and $\sigma_1$ from both populations are equal.
...
\noindent This estimated standard error $\ssigma_{\xmean-\ymean}$ is used as the estimate of $\ssigma_{\mu-\mu_1}$ with a t-Distribution with degrees of freedom which are also merely added:
