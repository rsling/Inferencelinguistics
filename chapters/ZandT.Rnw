...
This doesn't even meet the lowest standard for a unexpected result under the Null ($sig=0.05$), and hence we do not conclude anything from this result.
...
\subsubsection{Conditions for Testing Mean Differences}
